...
export default async function DashboardPage({
  ...
  const [
    ...
    // isDuplicate: false → les doublons détectés (bug Afrishop) sont exclus de toutes les stats
    prisma.order.count({ where: { isDuplicate: false, createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "NOUVEAU", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "CONFIRME", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "REPORTE", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "PDR", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "INJOIGNABLE", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "ANNULE", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "EN_LIVRAISON", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "LIVRE", createdAt: { gte: since, lte: until } } }),
    prisma.order.count({ where: { isDuplicate: false, status: "RETOURNE", createdAt: { gte: since, lte: until } } }),
    prisma.order.aggregate({ where: { isDuplicate: false, createdAt: { gte: since, lte: until } }, _sum: { totalAmount: true } }),
